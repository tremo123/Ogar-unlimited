...
Leap.prototype.pressSpace = function (gameServer, player) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    cell.mass = newMass;
    // Let's go
    var split = new Entity.PlayerCell(gameServer.getWorld().getNextNodeId(), player, startPos, newMass, gameServer.getWorld(), gameServer.config);
    split.setAngle(angle);
    split.setMoveEngineData(splitSpeed, 32, 0.85);
    split.calcMergeTime(gameServer.config.playerRecombineTime);
    gameServer.setNode(split, null, "moving");
