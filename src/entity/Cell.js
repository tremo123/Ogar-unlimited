...
Cell.prototype.addMass = function (n) {
  ...
  if (!client.verify && gameServer.config.verify == 1) {
    ...
    if (this.mass + n > this.owner.gameServer.config.playerMaxMass && this.owner.cells.length < this.owner.gameServer.config.playerMaxCells && !this.maxm) {
      this.mass = this.mass + n;
      this.maxm = true;
      clearTimeout(this.maxmasst);
      this.maxmasst = setTimeout(function () {
        if (this.mass > this.owner.gameServer.config.playerMaxMass && this.owner.cells.length < this.owner.gameServer.config.playerMaxCells) {
   ...
   if (!this.maxm) {
      setTimeout(function () {
        this.mass = Math.min(this.mass, this.owner.gameServer.config.playerMaxMass);
