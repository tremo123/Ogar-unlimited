...
MotherCell.prototype.checkEat = function(gameServer) {
    ...
    // Don't let mother cells get too big, they won't get smaller
    if(this.mass > gameServer.config.mCellMaxMass) {
        // Spew food based off of gameserver.ini
        if(gameServer.currentFood < gameServer.config.foodMaxAmount) {
            this.spawnBFood();
            this.mass -= gameServer.config.bMCFoodMass;
        } else {
...
MotherCell.prototype.spawnBFood = function(gameServer) {
    ...
    // Spawn food
    var f = new Food(gameServer.getNextNodeId(), null, pos, gameServer.config.bMCFoodMass, gameServer);
