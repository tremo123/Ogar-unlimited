...
MotherCell.prototype.checkEat = function(gameServer) {
    ...
    if(this.mass > gameServer.config.mCellMaxMass) {
        ...
        if(gameServer.currentFood < gameServer.config.foodMaxAmount) {
            this.spawnBFood();
            this.mass -= bMCFoodMass;
            if (this.mass > gameServer.config.mCellStartMass) {
                this.mass = gameServer.config.mCellStartMass;
            } else {
                this.mass = this.mass;
            }
        } else {
...
MotherCell.prototype.spawnBFood = function(gameServer) {
    ...
    // Spawn food
    var f = new Food(gameServer.getNextNodeId(), null, pos, gameServer.config.bMCFoodMass);
